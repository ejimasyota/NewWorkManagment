...
try {
    ...
    /** 削除対象の元号が存在するか確認 */
    $CheckSql = "SELECT caleId FROM CalendarInfo WHERE caleId = :caleId AND workId = :workId";
    $CheckStmt = $Pdo->prepare($CheckSql);
    $CheckStmt->execute([':caleId' => $CaleId, ':workId' => $WorkId]);
    $CalendarInfo = $CheckStmt->fetch();

    if (!$CalendarInfo) {
        Database::Rollback();
        Response::Error('削除対象の元号が見つかりません');
    }

    /** 元号を削除 */
    $Sql = "DELETE FROM CalendarInfo WHERE caleId = :caleId AND workId = :workId";

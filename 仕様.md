...
```
ご提示いただいた要件（特に「登録者カラムの追加」「日付のVARCHAR化」「未定義テーブルの追加」「相関図やスケジュールの仕様」）に基づき、PostgreSQL用のDDL（CREATE文）を再構築しました。
主な変更・改修点
 * 共通カラムの徹底: 全テーブルに registDate, updateDate (TIMESTAMP(3)), registUser, updateUser (VARCHAR) を追加しました。
 * 日付型の統一: ユーザが入力する日付（誕生日、設立日など）は要件通り VARCHAR（YYYY-MM-DD形式などに対応）としています。
 * 不足テーブルの追加:
   * ClassInfo: 作中階級設定
   * RelationInfo: 相関図（矢印・線）
   * RelationObjectInfo: 相関図（座標・グルーピング）
   * ScheduleInfo: 作成スケジュール
   * WorkMemoInfo: 作品作成用メモ
 * テーブル結合の適正化: 組織IDやキャラクターIDなど、リレーションが必要な箇所に外部キー制約を設定しました。
<!-- end list -->
/* =========================================================================
 * 初期設定：UUID拡張機能の有効化
 * ========================================================================= */
